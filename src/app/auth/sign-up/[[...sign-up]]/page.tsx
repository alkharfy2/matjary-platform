      ...
      <div className="grid w-full max-w-[1120px] overflow-hidden rounded-[32px] border border-[var(--ds-divider)] bg-[var(--ds-surface-glass)] shadow-[var(--ds-shadow-lg)] backdrop-blur lg:grid-cols-[1.05fr_0.95fr]">
        <Reveal className="relative hidden overflow-hidden bg-[linear-gradient(160deg,var(--ds-primary),color-mix(in_oklab,var(--ds-primary)_55%,var(--ds-accent)))] p-10 text-white lg:block">
          ...
          <div className="relative space-y-6">
            <span className="ds-pill w-fit border-white/20 bg-white/12 text-white/88">ابدأ من هنا</span>
            <div className="space-y-4">
              <h1 className="ds-heading text-4xl font-black leading-tight">أنشئ متجرك اليوم بشكل يليق بعلامتك</h1>
              <p className="max-w-md text-sm leading-8 text-white/82">
                سجّل الآن وابدأ من منصة واحدة تجمع التسويق، البيع، وإدارة المتجر في تجربة عربية أنيقة وسريعة.
              </p>
            </div>
            <div className="grid gap-3">
              <div className="glass-panel rounded-[24px] border-white/55 bg-white/90 p-4 text-[var(--ds-text)]">
                <p className="text-sm font-semibold">إطلاق أسرع</p>
                <p className="mt-1 text-sm text-[var(--ds-text-muted)]">ابدأ من onboarding واضح ثم خصص المتجر والخطة.</p>
              </div>
              <div className="glass-panel rounded-[24px] border-white/55 bg-white/90 p-4 text-[var(--ds-text)]">
                <p className="text-sm font-semibold">واجهة أكثر جذبًا</p>
                <p className="mt-1 text-sm text-[var(--ds-text-muted)]">منصة ومتجر ولوحة تحكم كلها مبنية على نفس اللغة البصرية.</p>
              </div>
            </div>
          </div>
        </Reveal>
